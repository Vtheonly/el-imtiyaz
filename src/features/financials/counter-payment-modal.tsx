...
export function CounterPaymentModal({
  ...
  useEffect(() => {
    if (presetInstallmentId) return; // don't override preset
  ...
  async function submit() {
    ...
    if (proofRequired && !proofFileName) {
      toast.showWarning("Justificatif requis", "Chèque et virement nécessitent un justificatif (plan §18.03).");
      return;
    }
    setSubmitting(true);
    try {
        ...
        {stage === "success" && receiptPayment && (
          <>
            <DialogHeader>
              ...
              <DialogDescription>Reçu généré automatiquement — prêt à partager.</DialogDescription>
            </DialogHeader>
