...
import type {
  NodeDefinition,
  NodeConfigField,
} from "../../../services/workflow/node-registry";
...
export function WorkflowBuilder({
  ...
  const handleCanvasMouseDown = (e: React.MouseEvent) => {
    if (e.button === 1 || (e.button === 0 && e.shiftKey)) {
      e.preventDefault();
      // middle button panning can be configured here
    }
  };
  ...
  const handleCanvasMouseUp = (e: React.MouseEvent) => {
    ...
    if (drag.type === "edge" && drag.fromPort) {
      ...
      const targetPort = targetElement.dataset.port;
      const targetNodeId = targetElement.dataset.nodeId;
      if (targetPort && targetNodeId && drag.fromPort.nodeId !== targetNodeId) {
        ...
        const exists = edges.some(
          (edge) =>
            ...
            edge.sourcePort === newEdge.sourcePort &&
            edge.target === newEdge.target &&
            edge.targetPort === newEdge.targetPort,
        );
        if (!exists) {
  ...
  const handleCanvasDrop = (e: React.DragEvent) => {
    ...
    const newNode: WorkflowNode = {
      ...
      subtype: def.id,
      label: def.label,
      config: def.configSchema.reduce(
        (acc, f) => {
  ...
  const groupedRegistry = nodeRegistry.reduce(
    (acc, def) => {
    ...
    <div className="flex h-full" style={{ overflow: "hidden" }}>
      ...
      >
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <Badge
              tone={workflow.status === "published" ? "success" : "neutral"}
            >
          ...
          >
            ...
            >
              ...
              {drag?.type === "edge" &&
                ...
                (() => {
                  const from = getPortPosition(
                    drag.fromPort.nodeId,
                    drag.fromPort.portId,
                    "output",
                  );
                  if (!from) return null;
            ...
            {nodes.map((node) => {
              ...
              const Icon = def
                ? ((Lucide as any)[def.icon] ?? Lucide.Square)
                : Lucide.Square;
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    cursor:
                      drag?.type === "node" && drag.nodeId === node.id
                        ? "grabbing"
                        : "grab",
                    transition:
      ...
      >
        <div style={{ lineHeight: 1.7, fontSize: "var(--text-sm)" }}>
          <p>
            <strong>Add nodes:</strong> Drag from the left palette onto the
            canvas.
          </p>
          <p>
            <strong>Move nodes:</strong> Click and drag.
          </p>
          <p>
            <strong>Connect nodes:</strong> Drag from an output port (right
            side) to an input port (left side).
          </p>
          <p>
            <strong>Delete:</strong> Select a node or edge, then press{" "}
            <kbd>Delete</kbd>.
          </p>
          <p>
            <strong>Edit configuration:</strong> Click a node — the inspector
            appears on the right.
          </p>
          <p>
            <strong>Publish:</strong> A workflow must have at least one trigger
            node and be published before it can run.
          </p>
        </div>
      </Modal>
    </div>
      ...
      {side === "left" && (
        <>
          <div
            ...
            style={{
              position: "absolute",
              left: -12 - PORT_RADIUS, // perfectly aligns with the exact bounds utilized by SVG renderer
              top: "50%",
      ...
      {side === "right" && (
        <>
          ...
          <div
            ...
            style={{
              position: "absolute",
              right: -12 - PORT_RADIUS, // perfectly aligns with the exact bounds utilized by SVG renderer
              top: "50%",
              transform: "translateY(-50%)",
              width: PORT_RADIUS * 2,
              height: PORT_RADIUS * 2,
